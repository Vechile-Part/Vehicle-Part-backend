    ...
    ) : IStaffService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            for (int attempt = 1; attempt <= maxAttempts; attempt++)
            {
                ...
                var lineItems = resolvedItems.Select(x => (
                    Item: new SalesInvoiceItem
                    {
                        PartId = x.Part.Id,
                        Quantity = x.Quantity,
                        UnitPrice = x.Part.UnitPrice
                    },
                    Part: x.Part
                )).ToList();
                ...
                try
                {
                    ...
                    var itemResponses = lineItems.Select(x => new SalesInvoiceItemResponseDto(
                        x.Item.Id, x.Part.Id, x.Part.Name,
                        x.Item.Quantity, x.Item.UnitPrice,
                        x.Item.Quantity * x.Item.UnitPrice)).ToList();
