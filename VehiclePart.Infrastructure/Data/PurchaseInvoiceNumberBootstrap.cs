...
public static class PurchaseInvoiceNumberBootstrap
{
    public static async Task BackfillMissingAsync(AppDbContext dbContext, CancellationToken cancellationToken = default)
    {
        ...
        var sequencesByYear = await dbContext.PurchaseInvoices.AsNoTracking()
            .Where(invoice => invoice.InvoiceNumber != null && invoice.InvoiceNumber != string.Empty)
        ...
        {
            if (!TryParseSequence(number, out var year, out var sequence))
                continue;
        ...
        foreach (var invoice in missing)
        {
            ...
            invoice.InvoiceNumber = $"PUR-{year}-{next:D3}";
        }

        await dbContext.SaveChangesAsync(cancellationToken);
    }

    private static bool TryParseSequence(string number, out int year, out int sequence)
    {
        year = 0;
        sequence = 0;
        if (!number.StartsWith("PUR-", StringComparison.OrdinalIgnoreCase))
            return false;
